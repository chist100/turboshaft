function [I_num_I_denom] = temperature_function(Start_T, Stop_T, Step_T, delta_num, delta_denom)
...
%}
    ...
    for N = Start_N:1:Stop_N
        ...
        for state = 0:1:3
            ...
            exp_num = exp(-1.4388*((Equilibrium_const(2, 1))*(state)-(Equilibrium_const(2, 2))*(state)^2-(Equilibrium_const(2, 2))*(state))/(T));
            snum(state+1) = factor_exp_num * exp_num;
            ...
            exp_denom = exp(-1.4388*((Equilibrium_const(2, 1))*(state)-(Equilibrium_const(2, 2))*(state)^2-(Equilibrium_const(2, 2))*state)/(T));
            sdenom(state+1) = factor_exp_denom * exp_denom;
        end
        denom(N) = sum(sdenom);
        num(N) = sum(snum);
