...
shild = Aperture( [ 165 0 0 ], [20.5 25] );
bench.append( shild ); 
...
y_cord = 0:1000/(number_nodes-1):1000;
% fiber_cord = zeros(size(y_cord));
% for i = 1:length(y_cord)
%     [~, fiber_cord(i)] = min(abs(y2 - y_cord(i)));
% end
%Диапазон координат волокна
startCoord = 250; %276.2
endCoord = 370; %359.2
Result_for_matlab = (startCoord:(endCoord-startCoord)/(number_nodes-1):endCoord);%fiber_cord/length(y2).*(366.5-276.2)+359.2;
